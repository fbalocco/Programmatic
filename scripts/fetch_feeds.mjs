...
/**
 * Fix common “invalid XML” issues seen in RSS feeds:
 * - unescaped & in attribute values / URLs
 * - illegal control characters
 */
function sanitizeXml(xml) {
  if (!xml) return xml;

  // Remove most ASCII control chars except tab/newline/carriage return.
...
async function loadConfig() {
  ...
  return {
    defaults: {
      ...
      userAgent: defaults.userAgent ?? 'Mozilla/5.0',
      fallbackUserAgent: defaults.fallbackUserAgent ?? null,
...
async function fetchText(url, { timeoutMs, headers, userAgent, fallbackUserAgent } = {}) {
  const doAttempt = async (ua) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const snippet = body ? body.slice(0, 220).replace(/\s+/g, ' ').trim() : '';
  ...
  try {
    return await doAttempt(userAgent);
  } catch (e) {
    const status = e?.status;
    const retryable = status === 403 || status === 429 || status === 502 || status === 503;
    if (retryable && fallbackUserAgent && fallbackUserAgent !== userAgent) {
      return await doAttempt(fallbackUserAgent);
    }
    throw e;
  }
}
...
async function main() {
  ...
  for (const feedDef of feeds) {
    ...
    const reqOpts = {
      timeoutMs: feedDef.fetch?.timeoutMs ?? defaults.timeoutMs,
      userAgent: feedDef.fetch?.userAgent ?? defaults.userAgent,
      fallbackUserAgent: feedDef.fetch?.fallbackUserAgent ?? defaults.fallbackUserAgent,
      headers: {
        Accept:
          'application/rss+xml, application/atom+xml, application/xml, text/xml, text/html;q=0.9, */*;q=0.8',
        ...defaults.headers,
        ...(feedDef.fetch?.headers || {}),
      },
    };
